...
export async function criarCredito(_prev: LancState, formData: FormData): Promise<LancState> {
  ...
  const { error } =
    tipo === 'empresa'
      ? await supabase.from('creditos_empresa').insert({
      ...
      : await supabase.from('creditos_pessoa').insert({
  ...
  redirect(`/creditos?tipo=${tipo}`)
}

export async function excluirCredito(
  tipo: TipoEntidade,
  id: number,
): Promise<{ error?: string } | void> {
  const supabase = await createClient()
  const tabela = tipo === 'empresa' ? 'creditos_empresa' : 'creditos_pessoa'
