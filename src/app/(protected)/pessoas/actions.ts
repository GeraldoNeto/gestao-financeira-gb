...
/**
 * Salva, pelo lado do irmão, o peso (%) que ele recebe de cada aluguel.
 * Substitui os vínculos deste irmão (apaga e reinsere os com peso > 0).
 * Campos do form: pct_<id_contrato> = peso.
 */
export async function salvarAlugueisIrmao(
  ...
  const rows: { id_contrato: number; id_pessoa: number; percentual: number }[] = []
  for (const [k, v] of formData.entries()) {
    if (!k.startsWith('pct_')) continue
    const idContrato = Number(k.slice(4))
    if (!Number.isInteger(idContrato)) continue
    let p = Number(String(v).replace(',', '.'))
    if (!Number.isFinite(p)) p = 0
    p = Math.min(100, Math.max(0, Math.round(p * 100) / 100))
    if (p > 0) rows.push({ id_contrato: idContrato, id_pessoa: idPessoa, percentual: p })
  }

  const supabase = await createClient()
  const del = await supabase.from('contrato_pessoa_percentual').delete().eq('id_pessoa', idPessoa)
  if (del.error) return { error: msgErroDB(del.error) }
  if (rows.length) {
    const ins = await supabase.from('contrato_pessoa_percentual').insert(rows)
